...
export const executeQuery = async ({ name, resolver, args = {} }) => {
  ...
  switch (key) {
    ...
    case 'Players.email':
      return {
        data: await apiGet(
          `/players/by-email?email=${encodeURIComponent(args.email)}`,
        ),
      };
    case 'Leagues.all':
    ...
    case 'Leagues.getBySlug':
      return {
        data: await apiGet(
          `/leagues/by-slug?slug=${encodeURIComponent(args.slug)}`,
        ),
      };
    default:
...
/**
 * A player's league-wide career stats (across every season in the league).
 * Not a reSolve view-model — a plain read keyed by two ids, so it gets its own
 * helper rather than a `fetchViewModel` case.
 */
export const fetchPlayerLeagueStats = (leagueId, playerId) =>
  apiGet(
    `/leagues/${encodeURIComponent(leagueId)}/players/${encodeURIComponent(playerId)}/stats`,
  );
...
export const fetchViewModel = async (name, aggregateId) => {
  switch (name) {
    ...
    case 'PlayerSettings': {
      const player = await apiGet(
        `/players/${encodeURIComponent(aggregateId)}`,
      );
      return {
        name: player?.name,
        deleted: !player,
