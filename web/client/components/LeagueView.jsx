...
const StarSvg = ({ filled, onClick, size }) => {
  ...
  return (
    <div
      onClick={onClick}
      title="Automatically browse to this league when logging in"
...
const LeagueView = ({ id, slug }) => {
  ...
  const { connect, dispose } = useViewModel('LeagueData', [id], setLeague);
  ...
  if (league?.currentSeason) {
    return (
      <div>
        <div className="bg-dark p-2 rounded mb-2 d-flex justify-content-between">
          ...
          {(() => {
            if (playerSettings.settings?.defaultLeague?.id === id) {
              return (
                <StarSvg filled={true} onClick={resetDefaultLeague}>
                  Default
                </StarSvg>
              );
            } else {
              return (
                <StarSvg filled={false} onClick={setDefaultLeague}>
                  Default
                </StarSvg>
              );
            }
        ...
        <SeasonView
          key={league.currentSeason}
          id={league.currentSeason}
          leagueId={id}
        ></SeasonView>
